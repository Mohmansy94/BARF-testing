...
class NeuralImageFunction(torch.nn.Module):
    ...
    def get_kernel(self, kernel_type, kernel_size, sigma=None, external_diff_sigma=None):
        ...
        match kernel_type:
            ...
            case "gaussian_diff":
                if external_diff_sigma is None:
                    external_diff_sigma = self.gaussian_diff_kernel_sigma
                kernel = kernels.get_gaussian_diff_kernel(kernel_size, external_sigma=external_diff_sigma)
            case "combined_diff":
                if external_diff_sigma is None:
                    external_diff_sigma = self.gaussian_diff_kernel_sigma
                kernel = kernels.get_combined_diff_kernel(kernel_size, external_sigma=external_diff_sigma, combine_levels = self.opt.arch.combine_levels)
            case "seperate":
